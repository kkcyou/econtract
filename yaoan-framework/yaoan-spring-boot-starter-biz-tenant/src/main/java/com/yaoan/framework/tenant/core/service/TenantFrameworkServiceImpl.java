package com.yaoan.framework.tenant.core.service;

import com.yaoan.framework.common.exception.ServiceException;
import com.yaoan.framework.common.util.cache.CacheUtils;
...
public class TenantFrameworkServiceImpl implements TenantFrameworkService {
    ...
    /**
     * 针对 {@link #getTenantPackageType(Long)} 的缓存
     */
    private final LoadingCache<Long, Integer> tenantPackageTypeCache = CacheUtils.buildAsyncReloadingCache(
            Duration.ofMinutes(1L), // 过期时间 1 分钟
            new CacheLoader<Long, Integer>() {
                ...
                public Integer load(Long id) {
                    try {
                        return tenantApi.getTenantPackageType(id);
                    } catch (ServiceException ex) {
                        return 0;
                    }
                }
    ...
    public Integer getTenantPackageType(Long id) {
        return tenantPackageTypeCache.getUnchecked(id);
    }
