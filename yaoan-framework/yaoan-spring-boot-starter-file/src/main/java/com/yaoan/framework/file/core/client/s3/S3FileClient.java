package com.yaoan.framework.file.core.client.s3;
...
import static com.yaoan.framework.file.core.client.s3.S3FileClientConfig.ENDPOINT_ALIYUN;
import static com.yaoan.framework.file.core.client.s3.S3FileClientConfig.ENDPOINT_TENCENT;
...
public class S3FileClient extends AbstractFileClient<S3FileClientConfig> {
    ...
    public String upload(byte[] content, String path, String type, String bucketName) throws Exception {
       ...
       ;
        // 执行上传
        client.putObject(builder // bucket 必须传递
                .contentType(type)
                .object(path) // 相对路径作为 key
                .stream(new ByteArrayInputStream(content), content.length, -1) // 文件内容
                .build());
        ...
        // 拼接返回路径
        return config.getEndpoint()+ "/"+ bucketName + "/" + path;
    }
