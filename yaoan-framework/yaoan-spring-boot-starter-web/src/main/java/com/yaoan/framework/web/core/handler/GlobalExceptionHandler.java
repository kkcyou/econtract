package com.yaoan.framework.web.core.handler;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.core.map.MapUtil;
import com.mysql.cj.jdbc.exceptions.CommunicationsException;
import com.yaoan.framework.apilog.core.service.ApiErrorLog;
import com.yaoan.framework.apilog.core.service.ApiErrorLogFrameworkService;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> allExceptionHandler(HttpServletRequest request, Throwable ex) {
        ...
        if (ex instanceof FeignException) {
            return feignExceptionHandler((FeignException) ex);
        }

        if (ex instanceof CommunicationsException) {
            return communicationsExceptionHandler((CommunicationsException) ex);
        }
        return defaultExceptionHandler(request, ex);
    }

    private CommonResult<?> communicationsExceptionHandler(CommunicationsException ex) {
        log.warn("[communicationsExceptionHandler]", ex);
        return CommonResult.error(JDBC_COMMUNICATION_ERROR.getCode(), String.format("服务器连接失败，请稍后重试。 ", ex.getLocalizedMessage()));
    }
    ...
    public CommonResult<?> feignExceptionHandler(FeignException ex) {
        log.warn("[feignExceptionHandler]", ex);
        return CommonResult.error(BAD_REQUEST.getCode(), String.format("服务对接失败，请检查 ", ex.getLocalizedMessage()));
    }
    ...
    public CommonResult<?> serviceExceptionHandler(ServiceException ex) {
        ...
        if (StringUtils.isBlank(message) || "系统异常".equals(message)) {
            message = "抱歉,服务器出错了";
        }
