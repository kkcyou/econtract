...
public enum OrderSourceEnums  {
    ...
    public static List<Integer> getOrderSource( String transactionMethod){
        if(ObjectUtil.isEmpty(transactionMethod)){
            return ListUtil.empty();
        }
        ArrayList<Integer> list = new ArrayList<>();
        for(OrderSourceEnums orderSourceEnums: values()){
            if(orderSourceEnums.getTransactionMethod().toString().equals(transactionMethod)){
                list.add(orderSourceEnums.getCode());
            }
        }
        return list;
    }
    ...
    public static String valuesOf(Integer code) {
        for (OrderSourceEnums model : values()) {
