...
依赖：docker compose v2，删除`name: yaoan-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name yaoan-maven-repo

docker run -it --rm --name yaoan-maven \
    -v yaoan-maven-repo:/root/.m2 \
