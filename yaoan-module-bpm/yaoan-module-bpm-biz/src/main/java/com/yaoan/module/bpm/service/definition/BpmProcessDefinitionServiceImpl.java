...
import com.yaoan.module.bpm.controller.admin.definition.vo.process.BpmProcessDefinitionPageReqVO;
import com.yaoan.module.bpm.dal.dataobject.definition.BpmFormDO;
import com.yaoan.module.bpm.dal.dataobject.definition.BpmProcessDefinitionInfoDO;
import com.yaoan.module.bpm.dal.mysql.definition.BpmProcessDefinitionInfoMapper;
...
import static com.yaoan.framework.common.util.collection.CollectionUtils.addIfNotNull;
import static com.yaoan.module.bpm.enums.ErrorCodeConstants.PROCESS_DEFINITION_KEY_NOT_MATCH;
import static com.yaoan.module.bpm.enums.ErrorCodeConstants.PROCESS_DEFINITION_NAME_NOT_MATCH;
...
public class BpmProcessDefinitionServiceImpl implements BpmProcessDefinitionService {
    ...
    public ProcessDefinition getActiveProcessDefinition(String key) {
        List<ProcessDefinition> list = repositoryService.createProcessDefinitionQuery()
                .processDefinitionKey(key).active().latestVersion().list();
        // 加入获得发起人的租户
        List<ProcessDefinition> filteredList = list.stream()
                .filter(processDefinition -> String.valueOf(SecurityFrameworkUtils.getLoginUser().getTenantId()).equals(processDefinition.getTenantId()))
                .collect(Collectors.toList());
        //拿到最新发布的定义
        return filteredList.stream().max(Comparator.comparing(ProcessDefinition::getVersion)).orElse(null);
    }
    ...
    @Override
    public ProcessDefinition getActiveProcessDefinition(String key, Long userId) {
        AdminUserRespDTO adminUserRespDTO = adminUserApi.getUser(userId);
        if (Objects.isNull(adminUserRespDTO)) {
        ...
        // 加入获得发起人的租户
        List<ProcessDefinition> filteredList = list.stream()
                .filter(processDefinition -> String.valueOf(adminUserRespDTO.getTenantId()).equals(processDefinition.getTenantId()))
                .collect(Collectors.toList());
        //拿到最新发布的定义
        return filteredList.stream().max(Comparator.comparing(ProcessDefinition::getVersion)).orElse(null);
    }
