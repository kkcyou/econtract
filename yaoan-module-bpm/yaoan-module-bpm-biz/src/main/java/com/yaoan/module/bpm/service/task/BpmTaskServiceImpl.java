package com.yaoan.module.bpm.service.task;
...
import com.yaoan.framework.common.util.object.PageUtils;
...
import com.yaoan.module.bpm.controller.admin.task.vo.task.withdraw.RevokeProcessRespVo;
import com.yaoan.module.bpm.controller.admin.task.vo.task.withdraw.RevokeProcessVo;
...
import com.yaoan.module.bpm.enums.definition.BpmUserTaskApproveTypeEnum;
import com.yaoan.module.bpm.enums.definition.BpmUserTaskAssignEmptyHandlerTypeEnum;
import com.yaoan.module.bpm.enums.definition.BpmUserTaskRejectHandlerType;
import com.yaoan.module.bpm.enums.definition.BpmUserTaskTimeoutHandlerTypeEnum;
import com.yaoan.module.bpm.enums.model.FlowableModelEnums;
import com.yaoan.module.bpm.enums.task.*;
import com.yaoan.module.bpm.framework.flowable.core.enums.BpmnVariableConstants;
...
import com.yaoan.module.bpm.service.definition.BpmModelService;
import com.yaoan.module.bpm.service.message.BpmMessageService;
...
import com.yaoan.module.econtract.api.listen.FileVersionEventPublisherApi;
...
import static com.yaoan.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.*;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    private static final String APPROVE = "审批通过";
    private static final String RETURN = "审批退回";
    @Resource
    ...
    // ========== Query 查询相关方法 ==========
    @Resource
    private FileVersionEventPublisherApi fileVersionEventPublisherApi;
    ...
    public PageResult<HistoricTaskInstance> getTaskPage(Long userId, BpmTaskPageReqVO pageVO) {
        ...
        if (StringUtils.isNotBlank(pageVO.getProcessDefinitionKey())) {
            taskQuery.taskDefinitionKey(pageVO.getProcessDefinitionKey());
        }
        if (StrUtil.isNotBlank(pageVO.getName())) {
            taskQuery.taskNameLike("%" + pageVO.getName() + "%");
        }
    ...
    public List<BpmTaskAllInfoRespVO> getAllTaskInfoRespByProcessInstanceIds(Long userId, List<String> processInstanceIds) {
        processInstanceIds = processInstanceIds.stream().filter(Objects::nonNull).collect(Collectors.toList());
        ...
        HistoricTaskInstanceQuery taskQuery = historyService.createHistoricTaskInstanceQuery()
                ...
                .taskTenantId(FlowableUtils.getTenantId())
                .orderByHistoricTaskInstanceEndTime().asc(); // 审批时间正序
    ...
    @Override
    public List<BpmTaskAllInfoRespVO> getAllTaskInfoRespByTaskIds(Long loginUserId, List<String> taskIds) {
        taskIds = taskIds.stream().filter(Objects::nonNull).collect(Collectors.toList());
    ...
    //    @Override
    public List<BpmTaskAllInfoRespVO> getToDoTaskInfoRespByProcessInstanceIds(Long userId, List<String> processInstanceIds) {
        processInstanceIds = processInstanceIds.stream().filter(Objects::nonNull).collect(Collectors.toList());
        StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM ACT_RU_TASK WHERE PROC_INST_ID_ IN (");
        // 构建 SQL IN 子句
        ...
        // 设置参数
        for (int i = 0; i < processInstanceIds.size(); i++) {
            nativeTaskQuery.parameter(String.valueOf(i + 1), processInstanceIds.get(i)); // 参数索引从1开始
        }
        // 执行查询并获取结果
        List<Task> runTasks = nativeTaskQuery.list();
        List<BpmTaskAllInfoRespVO> toDoTask = new ArrayList<BpmTaskAllInfoRespVO>();
        if (CollectionUtil.isNotEmpty(runTasks)) {
            for (Task runTask : runTasks) {
                BpmTaskAllInfoRespVO allInfoRespVO = new BpmTaskAllInfoRespVO();
                allInfoRespVO.setProcessInstanceId(runTask.getProcessInstanceId());
                allInfoRespVO.setAssigneeUserId(Long.valueOf(runTask.getAssignee()));
                allInfoRespVO.setCreateTime(DateUtils.of(runTask.getCreateTime()));
                allInfoRespVO.setTaskId(runTask.getId());
                toDoTask.add(allInfoRespVO);
            }
        }
        return toDoTask;
    }
    ...
    public List<HistoricTaskInstance> getTaskListByProcessInstanceId(String processInstanceId) {
        ...
        AtomicReference<List<HistoricTaskInstance>> taskReference = new AtomicReference<>(tasks);
        DataPermissionUtils.executeIgnore(() -> {
            List<HistoricTaskInstance> result = historyService.createHistoricTaskInstanceQuery()
                    ...
                    .list();
            taskReference.set(result);
        });
        tasks = taskReference.get();
    ...
    private Task validateTask(Long userId, String taskId) {
        ...
        if (StrUtil.isNotBlank(task.getAssignee())){
            if (ObjectUtil.notEqual(userId, NumberUtils.parseLong(task.getAssignee()))){
                // 如果不是自己的任务，判断是否是自己所在相对方联系人的任务，也不是就抛异常
                RelativeApi relativeApi = SpringUtil.getBean(RelativeApi.class);
                Long contactId = relativeApi.getRelativeContactId();
                if (ObjectUtil.notEqual(contactId , NumberUtils.parseLong(task.getAssignee()))){
...
//    }
    ...
    public void handleTask(BpmHandleTaskReqDTO bpmHandleTaskReqDTO) {
        BpmTaskApproveReqVO reqVO = BpmTaskConvert.INSTANCE.convertHandleDTO2Req(bpmHandleTaskReqDTO);
        Map<String, Object> processInstanceVariables = new HashMap<>();
        ContractProcessInstanceResultEnum resultEnum = ContractProcessInstanceResultEnum.valueOf(bpmHandleTaskReqDTO.getResult());
        if (ObjectUtil.isNotNull(resultEnum)) {
            processInstanceVariables.put("passVal", resultEnum.getCommand());
        }
        reqVO.setVariables(processInstanceVariables);
        // 1.1 校验任务存在
        Task task = validateTask(bpmHandleTaskReqDTO.getUserId(), reqVO.getId());
        // 1.2 校验流程实例存在
        ProcessInstance instance = processInstanceService.getProcessInstance(task.getProcessInstanceId());
        if (instance == null) {
            throw exception(PROCESS_INSTANCE_NOT_EXISTS);
        ...
        BpmTaskStatusEnum taskStatusEnum = BpmTaskStatusEnum.getInstance(bpmHandleTaskReqDTO.getResult());
        // 情况三：审批普通的任务。大多数情况下，都是这样
        // 3.1 更新 task 状态、原因
        updateTaskStatusAndReason(task.getId(), BpmTaskStatusEnum.APPROVE.getStatus(), reqVO.getReason());
        // 3.2 添加评论
    ...
    public void handleTask2(BpmHandleTaskReqDTO bpmHandleTaskReqDTO) {
        ...
        updateTaskStatusAndReason(bpmHandleTaskReqDTO.getTaskId(), BpmTaskStatusEnum.RETURN.getStatus(), bpmHandleTaskReqDTO.getReason());
        // 校验流程实例存在
        ProcessInstance instance = processInstanceService.getProcessInstance(task.getProcessInstanceId());
        if (instance == null) {
            throw exception(PROCESS_INSTANCE_NOT_EXISTS);
    ...
    public void approveTask(Long userId, @Valid BpmTaskApproveReqVO reqVO) {
        ...
        // 文件留痕V2
        sendEvent4FileVersion(reqVO.getId(), APPROVE);

        // 消息监听发送
        ContractApi contractApi = SpringUtil.getBean(ContractApi.class);
        contractApi.warningNotify(task.getProcessInstanceId());
    }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 设置参数记录该数据存在退回操作，用于会签操作时的判断
        runtimeService.setVariable(task.getProcessInstanceId(), "is_reject", true);
    ...
    public void returnTask(Long userId, BpmTaskReturnReqVO reqVO) {
        ...
        // 文件留痕V2
        sendEvent4FileVersion(reqVO.getId(), RETURN);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void returnTaskV2(Long userId, BpmTaskReturnV2ReqVO reqVO) {
        // 1.1 当前任务 task
        Task task = validateTask(userId, reqVO.getId());
        if (task.isSuspended()) {
            throw exception(TASK_IS_PENDING);
        ...
        // 2. 调用 Flowable 框架的回退逻辑
        BpmTaskReturnReqVO bpmTaskReturnReqVO = BeanUtils.toBean(reqVO, BpmTaskReturnReqVO.class);
        returnTask(task, targetElement, bpmTaskReturnReqVO);
        ...
        if (ObjectUtil.isNotNull(activityConfigurationEnum)) {
            ...
            switch (activityConfigurationEnum) {
                ...
                case PAYMENT_APPLICATION_APPROVE: {
                    PaymentApplicationBpmApi bpmApi = SpringUtil.getBean(PaymentApplicationBpmApi.class);
                    bpmApi.notifyApproveStatus(reqVO.getBusinessId(), BpmProcessInstanceResultEnum.BACK);
                    return;
                }
                //撤回付款延期申请
    ...
    public void sendEvent4FileVersion(String taskId, String nodeName) {
        if (StringUtils.isNotBlank(taskId)) {
            // 文件留痕V2
            HistoricTaskInstance historicTaskInstance = historyService.createHistoricTaskInstanceQuery().taskId(taskId).singleResult();
            if (ObjectUtil.isNotNull(historicTaskInstance)) {
                fileVersionEventPublisherApi.sendEvent(
                        new FileVersionEventDTO()
...
//                        .setTaskId(task.getId()).setTaskName(task.getName()).setAssigneeUserId(Long.parseLong(task.getAssignee())));
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void withdrawal(WithDrawalReqVo reqVo) {
        String taskId = reqVo.getTaskId();
        HistoricTaskInstance userTask = historyService.createHistoricTaskInstanceQuery().taskId(taskId).singleResult();
        if (ObjectUtil.isNull(userTask)) {
            throw exception(SYSTEM_ERROR, "流程任务不存在");
        }
        // 1,校验流程（先判断是否是最后一岗,下一岗是否已经处理）
        RevokeProcessRespVo checkResult = checkTask4Withdrawal(new RevokeProcessVo().setProcessInstanceId(userTask.getProcessInstanceId())
                .setUserId(String.valueOf(SecurityFrameworkUtils.getLoginUserId())));
        HistoricTaskInstance latestTask = checkResult.getLatestTask();
        if (!checkResult.getResult()) {
            throw exception(SYSTEM_ERROR, checkResult.getMessage());
        }
        String LatestTaskId = latestTask.getId();
        // 2,执行退回逻辑

        // 2.0 插入撤回的记录
        Task task = getTask(latestTask.getId());
        TaskEntityImpl taskEntity = (TaskEntityImpl) task;
        taskEntity.setCountEnabled(true);
        taskEntity.setOwner(null);

        // 记录撤回任务的当前用户
        taskEntity.setAssignee(String.valueOf(SecurityFrameworkUtils.getLoginUserId()));

        taskEntity.setName(FlowableModelEnums.WITHDRAWAL.getName());
        taskService.saveTask(taskEntity);
        updateTaskStatus(taskEntity.getId(),BpmTaskStatusEnum.WITHDRAWAL.getStatus());

        // 将撤回任务状态更新
//        taskService.complete(taskEntity.getId());

        // 2.1 更新 task 状态 + 原因
        updateTaskStatusAndReason(LatestTaskId, BpmTaskStatusEnum.RETURN.getStatus(), reqVo.getReason());

        // 2.2 设置流程变量节点驳回标记：用于驳回到节点，不执行 BpmUserTaskAssignStartUserHandlerTypeEnum 策略。导致自动通过
        runtimeService.setVariable(latestTask.getProcessInstanceId(),
                String.format(PROCESS_INSTANCE_VARIABLE_RETURN_FLAG, latestTask.getTaskDefinitionKey()), Boolean.TRUE);
        List<String> returnTaskKeyList = new ArrayList<>();
        returnTaskKeyList.add(latestTask.getTaskDefinitionKey());

        // 2.3 执行驳回
        runtimeService.createChangeActivityStateBuilder()
                .processInstanceId(userTask.getProcessInstanceId())
                .moveActivityIdsToSingleActivityId(returnTaskKeyList, // 当前要跳转的节点列表( 1 或多)
                        userTask.getTaskDefinitionKey()) // targetKey 跳转到的节点(1)
                .changeState();

        // 3, 删除被退回的任务痕迹
        if (CollectionUtil.isNotEmpty(checkResult.getLatestTaskList())) {
            for (HistoricTaskInstance taskInstance : checkResult.getLatestTaskList()) {
                //保留撤回记录
                if(!latestTask.getId().equals(taskInstance.getId())) {
                    historyService.deleteHistoricTaskInstance(taskInstance.getId());
                }
            }
        }
        if(ObjectUtil.isNotNull(reqVo.getBusinessFlag())) {
            ContractApi contractApi = SpringUtil.getBean(ContractApi.class);
            contractApi.withdrawal(taskEntity.getProcessInstanceId(),reqVo.getBusinessFlag());
        }
    }

    static final String WITHDRAL_ERROR = "抱歉,该审批已不可撤回";
    //RevokeProcessVo 是封装的对象 包含流程实例ID、撤回人ID、撤回意见等...这里只用上了这3个，就不多说
    public RevokeProcessRespVo checkTask4Withdrawal(RevokeProcessVo revokeVo) {
        RevokeProcessRespVo respVo = new RevokeProcessRespVo();
        if (StringUtils.isBlank(revokeVo.getProcessInstanceId())) {
            log.error("【工作流异常】流程实例ID不能为空！");
            return respVo.setResult(false).setMessage(WITHDRAL_ERROR);
        }
        List<HistoricTaskInstance> taskList = historyService.createHistoricTaskInstanceQuery()
                .processInstanceId(revokeVo.getProcessInstanceId())
                .list();
        if (CollectionUtil.isEmpty(taskList)) {
            log.error("【工作流异常】流程任务不存在！");
            throw exception(SYSTEM_ERROR, WITHDRAL_ERROR);
        }
        List<HistoricTaskInstance> taskNodes = getTaskNodes(taskList);
        Map<String, List<HistoricTaskInstance>> taskListMap = CollectionUtils.convertMultiMap(taskList, HistoricTaskInstance::getTaskDefinitionKey);
        //开始校验
        HistoricTaskInstance latestTask = (HistoricTaskInstance) taskNodes.get(0);
        if (ObjectUtil.isNotNull(latestTask.getEndTime())) {
            log.error("【工作流异常】当前流程已结束！");
            return respVo.setResult(false).setMessage(WITHDRAL_ERROR);
        }
        if (2 > taskNodes.size()) {
            log.error("【工作流异常】下一节点不存在！");
            throw exception(SYSTEM_ERROR, WITHDRAL_ERROR);
        }
        HistoricTaskInstance currentTask = (HistoricTaskInstance) taskNodes.get(1);
        List<HistoricTaskInstance> currentTasktList = taskListMap.get(currentTask.getTaskDefinitionKey());
        if (CollectionUtil.isNotEmpty(currentTasktList)) {
            List<String> assigneeIds = currentTasktList.stream().map(HistoricTaskInstance::getAssignee).distinct().collect(Collectors.toList());
            if (!assigneeIds.contains(revokeVo.getUserId())) {
                log.error("【工作流异常】当前流程已不在下级节点， 下一级任务已被处理！");
                return respVo.setResult(false).setMessage(WITHDRAL_ERROR);
            }
        }
        List<HistoricTaskInstance> toDoTaskList = taskList.stream()
                .filter(task -> task.getEndTime() == null) // 判断 endTime 是否为 null
                .collect(Collectors.toList()); // 收集为一个新的 List
        return respVo.setResult(true).setLatestTask(latestTask).setLatestTaskList(toDoTaskList);
    }

    private List<HistoricTaskInstance> getTaskNodes(List<HistoricTaskInstance> taskList) {

        // 1. 按照 taskDefinitionKey 分组并保留每组 startTime 最大的任务
        Map<String, HistoricTaskInstance> groupedTasks = taskList.stream()
                .collect(Collectors.toMap(
                        HistoricTaskInstance::getTaskDefinitionKey,  // 分组依据：taskDefinitionKey
                        Function.identity(),  // 任务本身
                        (existing, replacement) -> existing.getStartTime().after(replacement.getStartTime()) ? existing : replacement // 保留 startTime 最大的任务
                ));

        // 2. 获取分组后的任务列表，并按 startTime 降序排序
        List<HistoricTaskInstance> filteredAndSortedList = new ArrayList<>(groupedTasks.values());
        filteredAndSortedList.sort((task1, task2) -> task2.getStartTime().compareTo(task1.getStartTime())); // 降序排序
