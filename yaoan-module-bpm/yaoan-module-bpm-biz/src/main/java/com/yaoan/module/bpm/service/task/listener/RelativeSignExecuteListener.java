 ...
 */
@Slf4j
public class RelativeSignExecuteListener implements TaskListener {
        ...
        if ((isReject != null && isReject) || (committed != null && committed)){
            ...
            if (processInstance != null) {
                ...
                if (ContractProcessInstanceResultEnum.REJECT.getCommand().equals(processVariables.get("passVal"))){
                    // 如果是用户退回，判断该合同是否已有相对方签署，如果有则不允许退回
//                    contractApi.isContainsSigned(delegateTask.getProcessInstanceId());
                    log.info("触发了合同相对方确认退回任务，ProcessInstanceId:{}", delegateTask.getProcessInstanceId());
                    contractApi.updateRelativeSignReject(delegateTask.getProcessInstanceId(),"SIGN");
                    ...
                    contractApi.updateRelativeSignOrConfirm(delegateTask.getProcessInstanceId(),"SIGN");
                }
            }
        }
    }
