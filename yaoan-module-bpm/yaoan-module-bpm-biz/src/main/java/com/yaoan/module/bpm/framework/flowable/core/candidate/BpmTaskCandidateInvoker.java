...
import com.yaoan.module.bpm.enums.definition.BpmUserTaskAssignStartUserHandlerTypeEnum;
...
import static com.yaoan.module.bpm.enums.ErrorCodeConstants.MODEL_DEPLOY_FAIL_TASK_CANDIDATE_NOT_CONFIG;
import static com.yaoan.module.bpm.framework.flowable.core.enums.BpmnModelConstants.USER_TASK_PROCESS_INSTANCE_STARTER_COMPANY_ID;
...
public class BpmTaskCandidateInvoker {
    ...
    public Set<Long> calculateUsers(DelegateExecution execution) {
        ...
        if(BpmTaskCandidateStrategyEnum.ROLE.getStrategy().equals(strategy)){
            String startUserId = ((ExecutionEntityImpl) execution).getProcessInstance().getStartUserId();
            AdminUserRespDTO starter = adminUserApi.getUser(Long.valueOf(startUserId));
            if (ObjectUtil.isNotNull(starter)) {
                String companyId = String.valueOf(starter.getCompanyId());
                List<ExtensionElement> elements = new ArrayList<>();
                ExtensionElement element = new ExtensionElement();
                element.setElementText(companyId);
                elements.add(element);
                // 存入发起人的公司id
                execution.getCurrentFlowElement().getExtensionElements().put(USER_TASK_PROCESS_INSTANCE_STARTER_COMPANY_ID, elements);
            }
        }
    ...
    void removeDisableUsers(Set<Long> assigneeUserIds) {
        ...
        DataPermissionUtils.executeIgnore(()->{
            TenantUtils.executeIgnore(()->{
                ...
                List<Long> existRelativeVirtualIds = relativeApi.getExistRelativeVirtualIds(assigneeUserIds);
                relativeMap.put("existRelativeVirtualIds", existRelativeVirtualIds);
                Map<Long, AdminUserRespDTO>  t= adminUserApi.getUserMap(assigneeUserIds);
                userMap.set(t);
            });
        });
