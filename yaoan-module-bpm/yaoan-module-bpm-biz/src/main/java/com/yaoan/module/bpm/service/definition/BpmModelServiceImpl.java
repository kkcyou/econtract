package com.yaoan.module.bpm.service.definition;
...
import com.yaoan.module.bpm.controller.admin.definition.vo.model.*;
import com.yaoan.module.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelNodeVO;
import com.yaoan.module.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelUpdateReqVO;
import com.yaoan.module.bpm.convert.definition.BpmModelConvert;
import com.yaoan.module.bpm.dal.dataobject.definition.BpmFormDO;
import com.yaoan.module.bpm.enums.definition.BpmModelFormTypeEnum;
import com.yaoan.module.bpm.framework.flowable.core.candidate.BpmTaskCandidateInvoker;
...
import com.yaoan.module.bpm.framework.flowable.core.util.SimpleModelUtils;
...
public class BpmModelServiceImpl implements BpmModelService {
    ...
    public BpmModelListRespVO getModelList() {
        ...
        BpmModelListRespVO result = new BpmModelListRespVO();
        result.setDraftList(modelList.stream().filter(item -> item.getCategory().equals("contractDraft")).collect(Collectors.toList()));
        result.setRegisterList(modelList.stream().filter(item -> item.getCategory().equals("contractRegist")).collect(Collectors.toList()));
        result.setChangeList(modelList.stream().filter(item -> item.getCategory().equals("contractChange")).collect(Collectors.toList()));
        result.setPaymentList(modelList.stream().filter(item -> item.getCategory().equals("paymentProcess")).collect(Collectors.toList()));
        result.setCollectionList(modelList.stream().filter(item -> item.getCategory().equals("collectionProcess")).collect(Collectors.toList()));
        result.setBorrowList(modelList.stream().filter(item -> item.getCategory().equals("borrowProcess")).collect(Collectors.toList()));
        return result;
    }

    @Override
    public BpmModelRespVO getSimpleModelBykey(String key) {
        if (StringUtils.isBlank(key)) {
            log.error("流程定义不可为空，请联系管理员配置流程定义。");
            throw exception(DIY_ERROR, "请联系管理员配置流程定义。");
        }
        //获得流程信息
        ModelQuery modelQuery = repositoryService.createModelQuery();
        modelQuery.modelTenantId(FlowableUtils.getTenantId()).modelKey(key);
        // 执行查询
        long count = modelQuery.count();
        if (count == 0) {
            return new BpmModelRespVO();
        }
        Model model = modelQuery
                .orderByCreateTime().desc()
                .latestVersion().modelKey(key).singleResult();
        ...
        //获得流程模型
        byte[] bpmnBytes = getModelBpmnXML(model.getId());
        return BpmModelConvert.INSTANCE.buildModel(model, bpmnBytes);
    }
