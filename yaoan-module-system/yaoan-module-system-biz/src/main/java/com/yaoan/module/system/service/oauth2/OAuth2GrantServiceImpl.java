...
public class OAuth2GrantServiceImpl implements OAuth2GrantService {
    ...
    public OAuth2AccessTokenDO grantImplicit(Long userId, Integer userType,
                                             String clientId, List<String> scopes,Long companyId) {
        return oauth2TokenService.createAccessToken(userId, userType, clientId, companyId,scopes);
    }

    @Override
    public String grantAuthorizationCodeForCode(Long userId, Integer userType,
                                                String clientId, List<String> scopes,
                                                String redirectUri, String state,Long companyId) {
    ...
    public OAuth2AccessTokenDO grantAuthorizationCodeForAccessToken(String clientId, String code,
        ...
        return oauth2TokenService.createAccessToken(codeDO.getUserId(), codeDO.getUserType(),
                codeDO.getClientId(), companyId ,codeDO.getScopes());
    }
    ...
    public OAuth2AccessTokenDO grantPassword(String username, String password, String clientId, List<String> scopes) {
        ...
        // 创建访问令牌
        return oauth2TokenService.createAccessToken(user.getId(), UserTypeEnum.ADMIN.getValue(), clientId, user.getCompanyId(), scopes);
    }
    ...
    @Override
    public OAuth2AccessTokenDO grantUserIdToken(OAuth2ClientDO client, String userId, List<String> scopes) {

        // 创建访问令牌
        return oauth2TokenService.createBusinessKeyAccessToken(userId, UserTypeEnum.ADMIN.getValue(), client.getClientId(), scopes);
    }
