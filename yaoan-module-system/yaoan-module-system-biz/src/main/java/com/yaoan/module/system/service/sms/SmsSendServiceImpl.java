...
//        }
        //发送短信验证码需要redis模拟消息队列，用到XADD命令，需要redis5进行支持。此处先调整为直接发送
        if (isSend) {
            SmsSendMessage message = new SmsSendMessage().setLogId(sendLogId).setMobile(mobile).setContent(content);
            message.setChannelId(template.getChannelId()).setApiTemplateId(template.getApiTemplateId()).setTemplateParams(newTemplateParams);
            this.doSendSms(message);
        }
        return sendLogId;
    }
    ...
    public void doSendSms(SmsSendMessage message) {
        ...
        SmsCommonResult<SmsSendRespDTO> sendResult = smsClient.sendSms(message.getLogId(), message.getMobile(),
                message.getApiTemplateId(), message.getTemplateParams(),message.getContent());
