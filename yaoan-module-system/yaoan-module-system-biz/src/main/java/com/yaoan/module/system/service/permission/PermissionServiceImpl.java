...
public class PermissionServiceImpl implements PermissionService {
    ...
    @CacheEvict(value = RedisKeyConstants.MENU_ROLE_ID_LIST,
            allEntries = true) // allEntries 清空所有缓存，主要一次更新涉及到的 menuIds 较多，反倒批量会更快
    public void assignRoleMenuWithTenantId(Long roleId, Set<Long> menuIds, Long tenantId) {
        // 获得角色拥有菜单编号
        Set<Long> dbMenuIds = convertSet(roleMenuMapper.selectListByRoleId(roleId), RoleMenuDO::getMenuId);
        // 计算新增和删除的菜单编号
        Collection<Long> createMenuIds = CollUtil.subtract(menuIds, dbMenuIds);
    ...
    public void assignUserRole4Saas(Long userId, Set<Long> roleIds, String relativeId) {
        // 清除此用户在此相对方的所有关系
        userRoleMapper.deleteListByUserIdAndRelativeId(userId, relativeId);
        ...
        userRoleMapper.insertBatch(CollectionUtils.convertList(roleIds, roleId -> {
            ...
            // 注册无token，直接赋值默认值
            entity.setTenantId(SAAS_TENANT_ID);
            return entity;
        }));
    }
    ...
    @DSTransactional // 多数据源，使用 @DSTransactional 保证本地事务，以及数据源的切换
    @CacheEvict(value = RedisKeyConstants.USER_ROLE_ID_LIST, key = "#userId")
    public void assignUserRoleWithTenantId(Long userId, Set<Long> roleIds, Long tenantId) {
        // 获得角色拥有角色编号
        Set<Long> dbRoleIds = convertSet(userRoleMapper.selectListByUserId(userId),
                UserRoleDO::getRoleId);
        // 计算新增和删除的角色编号
    ...
    @Override
    public Set<Long> selectListByRoleIdsAndCompanyId(Collection<Long> roleIds) {
        return convertSet(userRoleMapper.selectListByRoleIdsAndCompanyId(roleIds), UserRoleDO::getUserId);
    }
    ...
    @Override
    public Set<Long> getUserRoleIdListByRoleIdV2(Collection<Long> roleIds) {
        return convertSet(userRoleMapper.selectListByRoleIds(roleIds), UserRoleDO::getUserId);
    }
    @Override
    public Set<Long> getUserRoleIdListByRoleIdsAndCompanyId(Collection<Long> roleIds,Long companyId) {
        return convertSet(userRoleMapper.getUserRoleIdListByRoleIdsAndCompanyId(roleIds, companyId), UserRoleDO::getUserId);
    }
    /**
    ...
    public DeptDataPermissionRespDTO getDeptDataPermission(Long userId) {
        ...
        for (RoleDO role : roles) {
            ...
            // 情况六，COMPANY
            if (Objects.equals(role.getDataScope(), DataScopeEnum.COMPANY.getScope())) {
